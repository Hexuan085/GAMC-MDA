...
def main():
    ...
    if use_gt:
        ...
        train_loader = instantiate(attack_config.dataset, train=True)
        valid_loader = instantiate(attack_config.dataset, train=False)
    else:
        print("Using predicted labels ..")
        train_loader = instantiate(attack_config.dataset, source_model=source_model, train=True)
        valid_loader = instantiate(attack_config.dataset, source_model=source_model, train=False)
    x_wm = train_loader.normalize(x_wm)  #only for unrelated, noise
    ...
    if "surrogate_model" in attack_config.keys():
        surrogate_model = instantiate(attack_config.surrogate_model)
        optimizer = instantiate(attack_config.optimizer, surrogate_model.parameters())
    ...
    attack: RemovalAttack = instantiate(attack_config.create, classifier=surrogate_model, config=attack_config)

    # Run the removal. We still need wrappers to conform to the old interface.
    start = time.time()
    attack, train_metric = instantiate(attack_config.remove,attack=attack,
    ...
    if args.save:
        ...
        print(surrogate_model.model == source_model.model)
        checkpoint = {
    ...
    print(f'surrogate_test_acc_after_attack: {surrogate_test_acc_after_attack}')
    print(f'surrogate_wm_acc_after_attack: {surrogate_wm_acc_after_attack}')
