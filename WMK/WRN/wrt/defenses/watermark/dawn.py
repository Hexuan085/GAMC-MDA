...
class Dawn(Watermark):
    ...
    def embed(self,
        ...
        train_loader: WRTDataLoader = instantiate(create_train_loader_fn, source_model=self.get_classifier(), train=True)

        surrogate_model: torch.nn.Sequential = instantiate(create_surrogate_model_fn)
        optimizer = instantiate(create_surrogate_optimizer_fn, surrogate_model.parameters())
        scheduler = instantiate(create_scheduler_fn, optimizer)
