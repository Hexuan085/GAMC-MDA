...
class Blackmarks(Watermark):
    ...
    def embed(self,
        ...
        # x_wm_1, y_wm_1, train_signature = self._generate_candidate_keys(wm_loader=wm_loader,
        #                                                             train_loader=train_loader,
        #                                                             encoding=self.encoding,
        #                                                             key_expansion_factor=key_expansion_factor,
        #                                                             keylength=keylength)
        
        state_dict = torch.load('/home/xhe085/Proj/NEW_WMK/Watermark-Robustness-Toolbox/outputs/cifar10/wm/blackmarks/00006_blackmarks/checkpoint.pth')
        x_wm_1, y_wm, self.encoding, train_signature, signature  = state_dict['x_wm_1'], state_dict['y_wm_1'], state_dict['encoding'], state_dict['train_signature_1'], state_dict['y_wm'] 
        print('y_wm_1',y_wm)
        
        self.visualize_key(x_wm_1, output_dir=output_dir)
        x_wm_2 = train_loader.normalize(x_wm_1)

        
        # Early stopping on the watermarking loss.
        callbacks.extend([wrt_callbacks.EarlyStoppingWRTCallback(
            lambda: self.classifier.evaluate(x_wm_2, y_wm)[0],
            check_every_n_batches=log_wm_acc_after_n_batches,
            patience=patience,
            mode='min'),
            wrt_callbacks.DebugWRTCallback(lambda: self.verify(x_wm_2, train_signature)[0],
                                           message="wm_acc",
                                           check_every_n_batches=log_wm_acc_after_n_batches)])
        ...
        self._embed_candidate_keys(train_loader, epochs=epochs, train_signature=train_signature, x_wm=x_wm_2, y_wm=y_wm,
                                   callbacks=callbacks)
        self.classifier.lr *= decrease_lr_by_factor

        # Filter candidates that are transferable or non-successful.
        x_wm_2, y_wm = self._filter_candidate_keys(train_loader, finetune_batches=finetune_batches, x_wm=x_wm_2, y_wm=y_wm)
        print(f"Generated {len(x_wm_2)} keys in total.. Now starting round two of the keygen.")

        # Sort keys into their group.
        group0, group1 = self._sort_keys_to_encoding(x_wm_2, y_wm)
        ...
        x_wm_2, y_wm = self._compose_key(group0, group1, signature)
        ...
        if len(x_wm_2) < len(signature):
            print(f"[WARNING]: Wanted {keylength} keys, but only generated {len(x_wm_2)}. "
                  f"Consider increasing the key_expansion_factor")

        return x_wm_2, signature[:len(x_wm_2)]
...
def wm_blackmarks_embed(defense: Blackmarks, wm_loader, config, **kwargs):
    wm_loader = instantiate(config.wm_dataset)
