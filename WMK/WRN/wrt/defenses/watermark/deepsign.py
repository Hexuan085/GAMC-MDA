...
class DeepSignWB(Watermark):
    ...
    def _get_mean_activation(self,
        ...
        def get_activations(x: np.ndarray):
            ...
            print('model_outputs', model_outputs[layer].shape)
            print('model_outputs', x.shape[0]) 
        ...
        if from_scratch:
            ...
            for target_class in tqdm(range(self.num_gaussians), desc="Initializing GMM.", disable=not verbose):
                ...
                if str(target_class) in self.X_key:
                    ...
                    target_class_loader: WRTDataLoader = instantiate(self.create_target_class_loader, class_labels=target_class)
    ...
    def embed(self,
        ...
        mu = self.get_activations(from_scratch=self.from_scratch, max_n=max(64, train_loader.batch_size), verbose=verbose) #max_n = 128
        print('mu_nan_1', True in np.isnan(mu))
        # Initialize the signature. (s \times N)
        # signature = np.random.randint(2, size=(self.num_gaussians, keylength)).astype(np.float32)
        signature =  np.ones((self.num_gaussians, keylength)).astype(np.float32)
        ...
        print('mu_nan_2', torch.isnan(mu).any())
        ...
        callbacks = [wrt_callbacks.DebugWRTCallback(lambda: self.verify(A, signature)[0],
                     ...
                     wrt_callbacks.DebugWRTCallback(lambda: trainer.evaluate()[1].value,
                                                    check_every_n_batches=check_every_n_batches),
                     wrt_callbacks.EarlyStoppingWRTCallback_Design(lambda: self.verify(A, signature)[0],
                                                            check_every_n_batches=check_every_n_batches,
                                                            patience=patience,
                                                            mode='max',
    ...
    def _train_step(self, x: torch.Tensor, y: torch.Tensor,
        ...
        if gamma2 > 0:
            # print("grad data", torch.isnan(mu.grad.data).any())
    ...
    def _embedding_procedure(self, train_loader: WRTDataLoader, num_gaussians: int, mu: torch.nn.Parameter,
        ...
        for e in range(nb_epochs):
            self.classifier.lr = self.schedule(e)
            print('self.classifier.lr', self.classifier.lr)
    ...
    def cal_loss2(mu: torch.Tensor,
        ...
        # print('mu_nan_3', torch.isnan(mu).any())
        G = torch.matmul(mu, A)
        G = torch.sigmoid(G)
