...
class Adi(Watermark):
    ...
    def visualize_key(self, x_wm: np.ndarray, output_dir: str = None):
        ...
        """
        idx = np.random.choice(np.arange(x_wm.shape[0]), size=2, replace=False)
    ...
    def keygen(wm_loader: data.DataLoader,
        ...
        if y_wm is None:
            y_wm = np.random.randint(0, 3, size=x_wm.shape[0])
            # y_wm = np.loadtxt('/home/xhe085/.wrt/data/labels-cifar.txt')
            print(x_wm.shape[0])
        return x_wm, y_wm
    ...
    def embed(self,
        ...
        np.savetxt(os.path.join(output_dir, "y_mk.txt"), y_wm, fmt="%d")
