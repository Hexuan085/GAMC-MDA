...
class EarlyStoppingWRTCallback_Design(WRTCallback):
    """ Early Stopping callback with a callable function to measure some metric. """
    def __init__(self,
                 measure_value_fn: Callable[[], float],
                 patience: int = 1,
    ...
    def _apply_step(self):
        ...
        if self._is_improvement(metric):
            ...
            self.current_patience = 0
            checkpoint_stopping = {
                'model': self.model.model.state_dict(), #for deepsign
                'optimizer': self.model.optimizer.state_dict(),
                'X_key': self.X_key,    # The watermarking key (a dict of images for each Gaussian)
                'x_wm': self.x_wm,   # The random projection matrix.
                'y_wm': self.y_wm,      # The signature provided by the user.
            }
            self.save('checkpoint.pth', path=self.output_dir, checkpoint=checkpoint_stopping)
        else:
            self.current_patience += 1
