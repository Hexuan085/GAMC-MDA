...
def parse_args():
    ...
    parser.add_argument("--filename", type=str, default="checkpoint.pth", help="Filepath to the pretrained model.")
...
def __load_model(model, optimizer, image_size, num_classes, pretrained_dir: str = None, best=True, load_optimizer=True,
    ...
    if pretrained_dir: #only for jia
...
def main():
    ...
    valid_loader_1 = instantiate(defense_config.dataset, train=False)
    # Optionally load a dataset to load watermarking images from.
    wm_loader = None
    if "wm_dataset" in dict(defense_config).keys():
        wm_loader = instantiate(defense_config.wm_dataset)
    ...
    (x_wm, y_wm), defense = instantiate(defense_config.embed, defense=defense,
    ...
    # Compute the outputs.
    metrics: dict = compute_metrics(defense, x_wm, y_wm, valid_loader_1)
