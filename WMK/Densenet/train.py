...
def __load_model(model, optimizer, resume, image_size, num_classes):
    ...
    if resume:
        ...
        print(f"Loading a pretrained source model from '{resume}'.")
        state_dict = torch.load(os.path.join(resume))
...
def main():
    ...
    model: torch.nn.Sequential = instantiate(config.model)
    model = model.to(device)

    optimizer = instantiate(config.optimizer, model.parameters())
    scheduler = instantiate(config.scheduler, optimizer)

    model: PyTorchClassifier = __load_model(model, resume = args.resume,
                                            optimizer=optimizer,
                                            image_size=config.model.image_size,
                                            num_classes=config.model.num_classes)

    train_loader = instantiate(config.dataset, train=True)
    valid_loader = instantiate(config.dataset, train=False)

    trainer = instantiate(config.trainer, model=model, train_loader=train_loader, valid_loader=valid_loader,
