...
def distillation_removal(attack: ModelDistillation,
                        ...
                        valid_loader: WRTDataLoader,
                        epochs: int = 60,
                        # scheduler: Optional = instantiate(config.scheduler),
                        callbacks: Optional[List[WRTCallback]] = None,
                        wm_data: Optional[Tuple] = None,
                        output_dir: Optional[str] = None,
                        **kwargs):
    optimizer = attack.get_classifier().optimizer
    return attack, attack.remove(source_model=source_model, train_loader=train_loader, valid_loader=valid_loader, epochs=epochs, callbacks=callbacks, wm_data=wm_data, output_dir=output_dir)
