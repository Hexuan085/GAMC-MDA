...
class Trainer(AbstractTrainer):
    ...
    def fit(self):
        ...
        for self.epoch in range(1, self.num_epochs + 1):
            self.model.lr = 0.01
            # self.model.lr = 0.0001 # for content
            self.model.lr = 0.001 #for deepmarks
    ...
    def resume(self, model, optimizer, f):
        checkpoint = torch.load(f)

        model.get_classifier.load_state_dict(checkpoint["model"])
